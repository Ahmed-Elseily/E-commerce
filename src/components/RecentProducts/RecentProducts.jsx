  ...
  async function addWishListProduct(productId) {
    ...
    if (response.data.status === 'success') {
      ...
      if (!wishlist.includes(productId)) {
        setWishlist((prevWishlist) => [...prevWishlist, productId]); // Add product ID to wishlist
      }
    } else {
      toast.error(response.data.message);
    }
            ...
            <div key={product.id} className='w-1/6 p-5'>
              <div className='main-product product-hover p-3 h-full'>
                ...
                <button
                  ...
                  className={`btn-wish outline outline-offset-0 outline-red-600 w-full mt-4 button-effect `}
                ...
                >
                  {loadingWishButton && currentProduct === product.id ? (
                    ...
                    <>
                      ...
                      <span className='ml-2 whitespace-nowrap'>{isInWishlist ? 'Added to Wishlist' : 'Add to Wishlist'}</span>
                    </>
                  )}
                </button>
              </div>
