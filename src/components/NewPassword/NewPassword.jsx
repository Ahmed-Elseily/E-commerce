...
export default function newPassword() {
  ...
  async function handleNewPassword(formValues) {
    setIsLoading(true);
  ...
  let yupValidation = Yup.object().shape({
    ...
    newPassword: Yup.string().matches(/^[A-Z][a-z0-9]{5,10}$/, "password should contain capital letters,lowercase letters and numbers").required("password is required"),
  })
    ...
    <>
      ...
      <div className=" w-full py-6 max-w-xl mx-auto">
        <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto">
          <h2 className='text-3xl font-bold mb-5 text-green-600'>Reset Password</h2>
          ...
          <div className="relative z-0 w-full mb-5 group">
            <input value={formik.values.newPassword} onChange={formik.handleChange} onBlur={formik.handleBlur} type="password" name="newPassword" id="password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " />
